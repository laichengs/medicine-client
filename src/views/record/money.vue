<template>
  <div class="container">
    <div class="filter-container" style="margin-bottom:10px;">
      ...
      <el-tag type="success" style="margin-left:10px;" v-if="money>0">总收入：{{money}}元</el-tag>
      <el-tag type="warning" style="margin-left:10px;" v-if="refund>0">总退款：{{refund}}元</el-tag>
    </div>
    <el-table :data="tableData" size="small" border v-loading="loading" style="width: 100%">
      ...
      <el-table-column align="center" prop="serialNo" label="订单编号"></el-table-column>
      ...
      <el-table-column align="center" prop="statusTitle" label="状态" width="100">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.status!=4" type="success">支付成功</el-tag>
          <el-tag v-if="scope.row.status==4" type="danger">退款成功</el-tag>
        </template>
      </el-table-column>
    </el-table>
    <pagination
  ...
  methods: {
    async init() {
      ...
      if (this.date.length > 0) {
        ...
        const { money, refund } = await getMoney({
          startDate: this.date[0] / 1000,
          endDate: this.date[1] / 1000 + 24 * 3600,
        })
        this.money = money
        this.refund = refund
      }
      const result = await getMoneyRecords(this.listQuery)
      this.loading = false
